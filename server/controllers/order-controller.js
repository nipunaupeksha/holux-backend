...
exports.createOrder = function (req, res, next) {
  ...
  let _query =
    'INSERT INTO orders(orderts, description, amount, userid, customerid, outletid) VALUES (now(),?,?,?,?,?)'
  dbConfig.query(
    _query,
    [description, amount, userid, customerid, outletid],
    (err, rows) => {
      if (err) {
...
exports.getDailyOrders = function (req, res, next) {
  ...
  let _query =
    'select o.orderid, o.orderts, o.description, o.amount, u.fullname, c.custname, ou.outletname from orders o inner join customer c on o.customerid=c.customerid inner join outlet ou on o.outletid=ou.outletid inner join user u on o.userid=u.userid where o.orderts like ?'
...
exports.getAnnualSales = function (req, res, next) {
  let _query =
    'select year(orderts) as year,month(orderts) as month,sum(amount) as sum from orders group by year(orderts),month(orderts) order by year(orderts),month(orderts)'
